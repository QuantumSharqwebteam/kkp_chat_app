...
class InquiryProvider with ChangeNotifier {
  ...
  Future<void> fetchInquiries({
    ...
    try {
      LoggingService.instance
          .logNetwork('Fetching inquiry forms for $cacheKey');
      final fetched =
      ...
      LoggingService.instance
          .logNetwork('Fetched ${fetched.length} inquiry forms for $cacheKey');
    } catch (e, stack) {
      LoggingService.instance.logNetwork(
  ...
  Future<void> refreshInquiries() async {
    await fetchInquiries(
        userEmail: _userEmail, role: _userRole, forceRefresh: true);
  }
  ...
  void _applyActiveFilters() {
    ...
    _filteredInquiries = _allInquiries.where((e) {
      ...
      if (_searchFilterActive && _searchText.trim().isNotEmpty) {
        ...
        matchesSearch = searchableValues
  ...
  // ================= MUTATIONS =================
  Future<void> updateFormStatus(String formId, String status,
      {String? reason}) async {
    try {
      await _chatRepository.updateInquiryFormStatus(formId, status,
          reason: reason);
      LoggingService.instance
          .logNetwork('Inquiry $formId status updated to $status');
  ...
  Future<void> updateFormRate(String formId, String rate) async {
    try {
      ...
      LoggingService.instance
          .logNetwork('Inquiry $formId rate updated to $rate');
