  ...
  Future<void> login(BuildContext context, String email, String password) async {
    ...
    try {
      await auth.login(email: email, password: password).then((value) async {
        if (value['message'] == 'User logged in successfully') {
          if (value['isVerified'] == true) {
            ...
            // User is not verified, show message and open verification bottom sheet
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text("You are not verified. Please verify your account first."),
                ),
              );
            }
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("Something Went Wrong !Pls try Again later"),
          ),
        );
      }
      debugPrint("Error: $e");
